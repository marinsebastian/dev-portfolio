    ...
    <section id="overview" className="relative min-h-[90vh] pt-28 pb-16 flex items-center bg-telemetry-grid bg-radial-glow overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          {/* Right Column: Executive Product Summary Card (RESUMEN PROFESIONAL - Increased Sizing) */}
          <motion.div
          ...
          >
            <div className="rounded-2xl border border-slate-800 bg-[#0f172a]/95 shadow-2xl p-6 lg:p-7 space-y-6">
              ...
              {/* High-Impact Executive Highlights (Increased Text Sizing) */}
              <div className="space-y-3.5 font-mono-tech text-xs sm:text-sm">
                
                <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 flex items-start space-x-3.5">
                  <Code2 className="w-5 h-5 text-teal-400 shrink-0 mt-0.5" />
                  <div>
                    <span className="text-slate-400 text-xs block font-mono-tech uppercase">{t('hero.telemetryDegreeLabel')}</span>
                    <span className="text-slate-100 font-bold text-xs sm:text-sm">{t('hero.telemetryDegree')}</span>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 flex items-start space-x-3.5">
                  ...
                  <div>
                    <span className="text-slate-400 text-xs block font-mono-tech uppercase">{t('hero.telemetryWorkLabel')}</span>
                    <span className="text-slate-100 font-bold text-xs sm:text-sm">{t('hero.telemetryWork')}</span>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 flex items-start space-x-3.5">
                  ...
                  <div>
                    <span className="text-slate-400 text-xs block font-mono-tech uppercase">{t('hero.telemetrySpecialtyLabel')}</span>
                    <span className="text-slate-100 font-bold text-xs sm:text-sm">{t('hero.telemetrySpecialty')}</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-1 font-mono-tech text-xs">
                <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800 text-center">
                  <span className="text-slate-400 block text-xs uppercase mb-1">{t('hero.telemetryQaLabel')}</span>
                  <span className="text-emerald-400 font-extrabold text-sm">{t('hero.telemetryQaValue')}</span>
                </div>
                <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800 text-center">
                  <span className="text-slate-400 block text-xs uppercase mb-1">{t('hero.telemetryDeployLabel')}</span>
                  <span className="text-teal-400 font-extrabold text-sm">{t('hero.telemetryDeployValue')}</span>
                </div>
              </div>
