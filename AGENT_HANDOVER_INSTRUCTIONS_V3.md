# AGENT_HANDOVER_INSTRUCTIONS_V3.md — Master Handover, AI Copilot & Enterprise UX Blueprint
...
**Document Version:** 3.0.0 (Enterprise UX, AI Copilot Function Calling & Live Micro-Apps)  
...
1. **CEO & CFO:** Look for business ROI, clear value proposition, commercial proof (Awtu Commerce with QR payment reconciliation), low-overhead open-source stack, and controlled AI API token costs.
...
3. **CTO:** Looks for clean Next.js 14 App Router architecture, TypeScript strict typing (`0 errors`), REST APIs (`/api/spatial`, `/api/php-sync`, `/api/gemini-assistant`), MapLibre GL + PMTiles vector GIS engine, interactive micro-apps (no static code blocks!), and automated Playwright QA runners.

---

## 2. Comprehensive Upgrade Roadmap & Architecture
...
### Module 2: AI Map Copilot & Focused Mode
...
- **IP Location Fallback:** Uses IP geolocation (`ipapi.co` / `ip-api.com`) to infer the candidate department (e.g. *Cochabamba* or *Santa Cruz*).
...
## 3. Summary of Round 1 Executive Reviews (`reviews/round-1/`)
...
## 4. Round 2 Review Execution Protocol for Incoming Agent
...
> ```markdown
> Please read `AGENT_HANDOVER_INSTRUCTIONS_V3.md` and `06_ENTERPRISE_UX_AND_AI_SPEC.md` in the root of `dev-portfolio`.
> 
> YOUR MANDATE:
> 1. Implement the Enterprise UX & AI Copilot enhancements detailed in `AGENT_HANDOVER_INSTRUCTIONS_V3.md`:
>    - **AI Map Copilot with Animated Gradient Border:** Triggers Focused Mode (Desktop: Side-by-side Map + Chat; Mobile: Map top / Chat bottom) with function calling map mutations and dynamic suggestion chips (`[Analizar mi barrio]`, `[Ver fibra > 80%]`).
